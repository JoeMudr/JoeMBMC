...
BMSModuleManager::BMSModuleManager(){
  ...
  lowestPackTemp = 200.0f; //[ToDo] Wird nie genutzt. Für Statistik vllt. neue Getter analog zu PackVolt einrichten
  highestPackTemp = -100.0f; //[ToDo] Wird nie genutzt. Für Statistik vllt. neue Getter analog zu PackVolt einrichten
  isFaulted = false;
}
...
int BMSModuleManager::seriescells(){
  spack = 0;
  for (int y = 1; y <= MAX_MODULE_ADDR; y++){
    if (modules[y].isExisting()){spack = spack + modules[y].getscells();}
  }
  return spack;
}

void BMSModuleManager::balanceCells(int duration, float hyst, int debug){
  uint8_t payload[4];
  uint8_t buff[30];
  ...
  for (int y = 1; y <= MAX_MODULE_ADDR; y++){
    if (modules[y].isExisting()){
      balance = 0;
      for (int i = 0; i < 6; i++){
        if (getLowCellVolt() + hyst < modules[y].getCellVoltage(i)){
          balance = balance | (1 << i);
          CellsBalancing++;
          }
      }
      if (balance != 0){ //only send balance command when needed
        ...
        delay(2);
        BMSUtil::getReply(buff, 30);
        balancingCells[y] = balance;
      }
    }
  }
}

byte BMSModuleManager::getBalancingCells(byte module_id){ //Welchen Sinn hat diese Funktion?
  if (modules[module_id].isExisting()){
    return balancingCells[module_id];
  }else{
    return 254;
  }
}
...
void BMSModuleManager::StopBalancing(){
  ...
  CellsBalancing = 0; //[JoeM] reset Balacing
}

//This Function has to be looped to use getters for all Values from the Modules
void BMSModuleManager::getAllVoltTemp(){ 
  ...
  BMSModuleManager::StopBalancing();
  delay(200);
  for (int x = 1; x <= MAX_MODULE_ADDR; x++){
    if (modules[x].isExisting()){
...
float BMSModuleManager::getLowVoltage(){
  if (BMSModuleManager::getPackVoltage() < lowestPackVolt) lowestPackVolt = BMSModuleManager::getPackVoltage();
  return lowestPackVolt;
}

float BMSModuleManager::getHighVoltage(){
  if (BMSModuleManager::getPackVoltage() > highestPackVolt) highestPackVolt = BMSModuleManager::getPackVoltage();
...
float BMSModuleManager::getAvgTemperature(){
  ...
  for (int x = 1; x <= MAX_MODULE_ADDR; x++){
    if (modules[x].isExisting()){
      if (modules[x].getAvgTemp() > -70){avg += modules[x].getAvgTemp();}else{y++;}
    }
  }
    ...
    if (modules[y].isExisting()){
      ...
      */
      ...
      for (int i = 0; i < 6; i++){
        ...
        SERIALCONSOLE.print("V");
        if (balancingCells[y] & (1 << i)){
          SERIALCONSOLE.print("*");
        }else{
          SERIALCONSOLE.print(" ");
        }
      }
      SERIALCONSOLE.print("  Temp1: ");
      SERIALCONSOLE.print(modules[y].getTemperature(0));
      SERIALCONSOLE.print("C  Temp2: ");
